...
<4> The http://docs.micronaut.io/snapshot/api/io/micronaut/http/annotation/Get.html[@Get] annotation is used to map the `index` method to GET `/` requests.
...
- The https://docs.micronaut.io/snapshot/api/io/micronaut/views/model/security/SecurityViewModelProcessor.html[SecurityViewModelProcessor] injects into the model a `security` map with the authenticated user. See https://micronaut-projects.github.io/micronaut-views/snapshot/guide/index.html#views-security[User in a view] documentation.
