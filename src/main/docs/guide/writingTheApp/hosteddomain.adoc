...
and then implement a `OpenIdClaimsValidator` bean. Micronaut validateds the id token against every bean of type `OpenIdClaimsValidator`. If the id token `hd` claim does not match the value configured, it is considered invalid.
